...
import os
import pwem
from protenix.constants import PROTENIX,PROTENIX_ENV_NAME, PROTENIX_DEFAULT_VERSION
...
class Plugin(pwem.Plugin):
    ...
    def getProtenixEnvActivation(cls):
        return "conda activate protenix-%s" %PROTENIX_DEFAULT_VERSION
    ...
    def runProtenix(cls, protocol, protenix, args, cwd=None):
        """ Run ProteniX command from a given protocol. """
        fullProgram = '%s %s && %s' % (cls.getCondaActivationCmd(),
                                       cls.getProtenixEnvActivation(), protenix)
